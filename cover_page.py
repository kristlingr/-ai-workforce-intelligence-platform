...
<style>
    ...
    .pill {
        display: inline-block; background: rgba(96,165,250,0.12); color: #93C5FD;
        font-size: 0.7rem; font-weight: 600; padding: 3px 10px; border-radius: 12px;
        border: 1px solid rgba(96,165,250,0.2); text-transform: uppercase; letter-spacing: 0.04em;
    }
...
with col2:
    st.markdown(
        ...
        f'<button class="launch-btn" onclick="window.location.href=\'/\'">'
        f'🚀 Launch Intelligence Dashboard</button></a></div>',
        unsafe_allow_html=True
    )
...
with col2:
    st.markdown(
        ...
        f'<a href="/" target="_self">'
        f'<button class="launch-btn" onclick="window.location.href=\'/\'">'
        f'🚀 Launch Intelligence Dashboard</button></a>'
        f'<div style="color:#475569; font-size:0.75rem; margin-top:0.8rem;">'
        f'No API keys required — runs in Enterprise Demo Environment mode</div>'
        f'</div>',
        unsafe_allow_html=True
    )
