...
class ForecastTool(BaseTool):
    ...
    def run(self, department: Optional[str] = None, months: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            df_attend, _ = load_attendance(strict=False)
            ...
            # 3. Calculate historical leave rate from attendance data
            leave_rate = 0.0
            if not df_attend.empty:
                df_attend_filtered = df_attend[df_attend["employee_id"].isin(target_emp_ids)]
                if not df_attend_filtered.empty:
                    weekday_records = df_attend_filtered[~df_attend_filtered["status"].str.lower().str.contains("weekend|holiday", na=True)]
                    if not weekday_records.empty:
                        leave_records = weekday_records[weekday_records["status"].str.lower().isin(["pto", "sick leave", "on leave", "absent"])]
                        leave_rate = len(leave_records) / len(weekday_records)
            
            logger.info(f"Calculated historical leave rate for department '{department or 'All'}': {leave_rate:.2%}")
            ...
            # Map YYYY-MM to Month Name YYYY (e.g. "2026-05" -> "May 2026")
            def get_month_display(month_str: str) -> str:
                try:
                    return pd.to_datetime(f"{month_str}-01").strftime("%B %Y")
                except Exception:
                    return month_str

            # 5. Perform month-by-month forecasting
            monthly_forecasts = []
            for month in target_months:
                ...
                # Compute available capacity (in hours)
                if not month_cap.empty:
                    available_capacity = float(month_cap["available_hours"].sum())
                    total_capacity_hours = float(month_cap["total_capacity_hours"].sum())
                else:
                    # Estimate working days (default 21 if unknown)
                    working_days = 21
                    standard_hours = 8.0
                    total_capacity_hours = len(target_emp_ids) * working_days * standard_hours
                    # Deduct hours based on leave rate
                    available_capacity = total_capacity_hours * (1.0 - leave_rate)

                capacity_fte = float(len(target_emp_ids))

                # Filter allocations that overlap with this month
                forecasted_workload = 0.0
                demand_fte = 0.0
                ...
                if not df_alloc.empty:
                    ...
                    # Calculate forecasted workload in hours: sum of (employee_available_hours * allocation_percentage)
                    for _, row in overlapping_allocs.iterrows():
                        emp_id = row["employee_id"]
                        alloc_pct = float(row["allocation_percentage"])
                        
                        # Find employee specific available hours for this month
                        emp_cap = month_cap[month_cap["employee_id"] == emp_id]
                        emp_avail_hours = float(emp_cap["available_hours"].iloc[0]) if not emp_cap.empty else (total_capacity_hours / len(target_emp_ids)) * (1.0 - leave_rate)
                        
                        forecasted_workload += emp_avail_hours * alloc_pct
                ...
                # Calculate Gaps
                resource_gap = round(forecasted_workload - available_capacity, 1)
                utilization = round((forecasted_workload / available_capacity * 100), 1) if available_capacity > 0 else 0.0
                
                net_fte_gap = round(capacity_fte - demand_fte, 2)
                status = "Shortage" if resource_gap > 0.0 else ("Surplus" if net_fte_gap > 0.5 else "Optimal")

                monthly_forecasts.append({
                    "month": get_month_display(month),
                    "month_key": month,
                    "available_capacity": round(available_capacity, 1),
                    "available_capacity_hours": round(available_capacity, 1),
                    "forecasted_workload": round(forecasted_workload, 1),
            ...
            # 6. Identify future bench dates (allocations ending)
            bench_releases = []
            if not df_alloc.empty:
                df_alloc_parsed = df_alloc.copy()
                df_alloc_parsed["start_date"] = pd.to_datetime(df_alloc_parsed["start_date"], errors="coerce")
            ...
            # Compute historical actual hours reference if worklogs are available
            historical_logged_hours = {}
            if not df_work.empty:
                df_work_filtered = df_work[df_work["employee_id"].isin(target_emp_ids)].copy()
                if not df_work_filtered.empty:
                    df_work_filtered["month"] = pd.to_datetime(df_work_filtered["date"]).dt.strftime("%Y-%m")
                    logged_by_month = df_work_filtered.groupby("month")["hours_logged"].sum().to_dict()
                    historical_logged_hours = {get_month_display(m): float(h) for m, h in logged_by_month.items()}
            ...
            return {
                ...
                "forecast": {
                    ...
                    "insights": insights,
                    "historical_logged_hours_reference": historical_logged_hours,
